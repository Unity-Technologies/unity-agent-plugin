...
|---------------|----------|
| `.uxml` or `.uss` files (including in `/Editor/`) | `/unity:ui-uitk` |
| "UI Toolkit", "UITK", "UIElements", "CreateGUI" | `/unity:ui-uitk` |
| Canvas prefabs/objects, `.prefab` with UI | `/unity:ui-ugui` |
| "uGUI", "Canvas", "RectTransform", "legacy UI" | `/unity:ui-ugui` |
| "IMGUI", "OnGUI", "OnInspectorGUI", "immediate mode" | `/unity:ui-imgui` |
...
- If no existing pattern is detected, default to `/unity:ui-uitk` for new editor UI
- Only use `/unity:ui-imgui` if project exclusively uses IMGUI or user explicitly requests it
...
- For new runtime/game UI where the user has no preference: default to uGUI (`/unity:ui-ugui`)
- When the user mentions mobile/performance constraints or older Unity versions (pre-6.0): bias toward uGUI (`/unity:ui-ugui`)
...
2. **Activate specialized skill** — Route to `/unity:ui-uitk`, `/unity:ui-ugui`, or `/unity:ui-imgui`
